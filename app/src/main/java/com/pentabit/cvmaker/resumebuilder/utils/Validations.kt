...
object Validations {
    ...
    fun validateFields(
        ...
        if (binding.nameedittext.toString().isNullOrEmpty()) {
            binding.nameTextInputLayout.error=("Please your name.")
            return false
        }
        ...
        /*if (binding.emailtext.toString().contains(".com")) {
            binding.textInputLayout3.error=("Please enter a valid email address.")
            return false
        }*/

        if (binding.phoneedittext.text.toString().length > 16) {
            binding.textInputLayout4.error=("enter phone number less than 16 digits")
            return false
        }
        if (binding.phoneedittext.text.toString().length <= 6) {
            binding.textInputLayout4.error=("enter phone number more than 6 digits")
            return false
        }
    ...
    fun isConditionMetEducation(binding: FragmentAddEducationBinding): Boolean {
        ...
        return when {
            ...
            !isEndDateRequired && endDate.isNullOrEmpty() -> {
                binding.enddateTextInputLayout2.error=("End date is required if the 'Continue' checkbox is unchecked.")
                false
            }

            startDate == endDate -> {
                binding.startdateTextInputLayout2.error=("Start date and end date should not be the same.")
                false
            }

            !endDate.isNullOrEmpty() && startDate > endDate -> {
                binding.enddateTextInputLayout2.error=("Start date should not be greater than End date.")
                false
            }
    ...
    fun isConditionMetExperience(binding: FragmentAddExperienceBinding): Boolean {
        ...
        return when {
            ...
            startDate == endDate -> {
                binding.enddateTextInputLayout2.error=("Start date and end date should not be the same.")
                false
            }

            !isEndDateRequired && endDate.isNullOrEmpty() -> {
                binding.enddateTextInputLayout2.error=("End date is required if 'Continue' is unchecked.")
                false
            }

            !endDate.isNullOrEmpty() && startDate > endDate -> {
                binding.startdateTextInputLayout2.error=("Start date should not be greater than End date.")
                false
            }
